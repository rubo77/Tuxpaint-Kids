/*
  ...
  Modified for Tuxpaint-Android to add EGL context synchronization
...
#define EGL_SYNC_TAG "Tux-EGL-Sync"
#define LOGI(...) __android_log_print(ANDROID_LOG_INFO, EGL_SYNC_TAG, __VA_ARGS__)
#define LOGE(...) __android_log_print(ANDROID_LOG_ERROR, EGL_SYNC_TAG, __VA_ARGS__)
...
/* Initialize the EGL context synchronization */
static void SDL_EGL_InitSync(void) {
...
/* Lock the EGL context */
static void SDL_EGL_LockContext(void) {
    if (!egl_mutex_initialized) {
        SDL_EGL_InitSync();
    }
...
/* Unlock the EGL context */
static void SDL_EGL_UnlockContext(void) {
...
static void SDL_EGL_TrackContextThread(void* context) {
    if (context) {
...
/* Original SDL_EGL function that we're overriding */
extern int SDL_EGL_MakeCurrent_Original(_THIS, EGLSurface egl_surface, SDL_GLContext context);

/* Our synchronized version that will be used instead */
int SDL_EGL_MakeCurrent(_THIS, EGLSurface egl_surface, SDL_GLContext context)
{
    ...
    /* Lock before changing context to prevent multiple threads from using it simultaneously */
    SDL_EGL_LockContext();
    LOGI("SDL_EGL_MakeCurrent called by thread %lu with context %p and surface %p", 
         (unsigned long)pthread_self(), egl_context, egl_surface);
    ...
    if (!egl_context || (!egl_surface && !_this->gl_allow_no_surface)) {
        result = _this->egl_data->eglMakeCurrent(_this->egl_data->egl_display, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT);
        SDL_EGL_TrackContextThread(NULL);  /* Clear context ownership */
        
        if (!result) {
            EGLint egl_error = _this->egl_data->eglGetError();
            LOGE("eglMakeCurrent(NULL) failed with error 0x%X", egl_error);
        }
    } else {
        /* Track which thread owns this context before making it current */
        SDL_EGL_TrackContextThread(egl_context);
        
        result = _this->egl_data->eglMakeCurrent(_this->egl_data->egl_display,
                                             egl_surface, egl_surface, egl_context);
        if (!result) {
            EGLint egl_error = _this->egl_data->eglGetError();
            LOGE("eglMakeCurrent failed with error 0x%X for context %p on thread %lu", 
                 egl_error, egl_context, (unsigned long)pthread_self());
            SDL_EGL_UnlockContext();
            return SDL_EGL_SetError("Unable to make EGL context current", "eglMakeCurrent");
        }
    }
    
    /* Unlock after context change */
    SDL_EGL_UnlockContext();
...
/* Original SDL_EGL function that we're overriding */
extern int SDL_EGL_SwapBuffers_Original(_THIS, EGLSurface egl_surface);
...
int SDL_EGL_SwapBuffers(_THIS, EGLSurface egl_surface)
{
    ...
    if (!_this->egl_data->eglSwapBuffers) {
        return SDL_SetError("EGL not initialized"); /* something clearly went wrong somewhere. */
    }
    
    /* Lock before swapping buffers to ensure thread safety with context */
    SDL_EGL_LockContext();
    LOGI("SDL_EGL_SwapBuffers called by thread %lu with surface %p", 
         (unsigned long)pthread_self(), egl_surface);
    
    result = _this->egl_data->eglSwapBuffers(_this->egl_data->egl_display, egl_surface);
    ...
    if (result != EGL_TRUE) {
        EGLint egl_error = _this->egl_data->eglGetError();
        LOGE("eglSwapBuffers failed with error 0x%X on thread %lu", 
             egl_error, (unsigned long)pthread_self());
